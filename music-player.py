...
#Add as many songs to the playlist as possible 

def addsongs():
    #to open a file
    temp_song=filedialog.askopenfilenames(initialdir="Music/"t,title="choose a song",filetypes=(("mp3 Files","*.mp3"),)):
    #loop through every item in the list to insert in the listbox
for s in temp_song:
    s=s.replace("C:/Users/DataFlair/Python_mp3-music-player/","")
...
def Play():
    song=songs_list.get(ACTIVE)
    song=f"C:/Users/Hp/Laptop/DataFlair/VsCode/Music/{song}"
    mixer.music.load(song)
    mixer.music.play()
...
#Function to navigate current song

def Previous():
    #To get to the selected song
    previous_one=songs_list.curselection()
    #to get the previous song
    previous_one=previous_one[0]-1
    #to get the previous song
    temp2=songs_list.get(previous_one)
    temp2=f"C:/User/DataFlair/python-mp3-music-player/{temp2}"
    mixer.music.load(temp2)
    mixer.music.play()
    songs_list.selection_clear(0.END)
    #activate new song
    songs_list.activate(previous_button)
    #play the next song
    songs_list.selection_set(previous_one)

def Next():
    #to get the selected song
    next_one=songs_list.curselection()
    #to get the next song
    next_one=next_one[0]+1
    #to get the next song
